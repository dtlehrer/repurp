...
import repurp.Variables;
import repurp.org.apache.commons.exec.launcher.CommandLauncher;
...
public class DefaultExecutor implements Executor {
	...
	/**
	 * Default constructor creating a default {@code PumpStreamHandler} and sets
	 * the working directory of the subprocess to the current working directory.
	 *
	 * The {@code PumpStreamHandler} pumps the output of the subprocess into our
	 * {@code System.out} and {@code System.err} to avoid into our
	 * {@code System.out} and {@code System.err} to avoid a blocked or
	...
	/**
	 * Execute an internal process. If the executing thread is interrupted while
	 * waiting for the child process to return the child process will be killed.
	 *
	 * @param command
	 ...
	 * @param streams
	 *            process the streams (in, out, err) of the process
	 * @return the exit code of the process
	 * @throws IOException
	 *             executing the process failed
	 */
	private int executeInternal(final CommandLine command, final Map<String, String> environment, final File dir,
		...
		try {
			...
			while (input.ready()) {
				...
				if (line.startsWith("  <Count>")) {
					...
					String r = line.substring(9, end);
					double i = Double.parseDouble(r);
					Variables.queryCount = i;
				}
			}
			input.close();
